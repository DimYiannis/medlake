<template>
  <div class="min-h-screen pt-36 pb-24" style="background:var(--bg)">
    ...
    <div class="px-10 mb-16 border-b pb-12" style="border-color:var(--border)">
      ...
      <p class="text-[14px] mt-5" style="color:var(--text-3)">Vertragsbedingungen Medlake Training AG — 2026/2027</p>
    </div>
    ...
    <div class="px-10 max-w-2xl">
      <div class="space-y-px" style="border-top:1px solid var(--border)">
        <div
          ...
          :key="i"
          class="flex gap-8 py-7 border-b"
          style="border-color:var(--border)"
        >
          <span class="text-[13px] font-medium flex-shrink-0 w-6 pt-0.5" style="color:var(--text-3)">{{ i + 1 }}.</span>
          <p class="text-[14px] leading-[1.85]" style="color:var(--text-2)">{{ article }}</p>
        </div>
      </div>

      <div class="mt-12 pt-10 border-t" style="border-color:var(--border)">
        <p class="text-[13px] leading-[1.8]" style="color:var(--text-3)">
          Bei Fragen zu unseren Vertragsbedingungen stehen wir Ihnen gerne zur Verfügung.
        </p>
        <a href="mailto:info@medlake.ch"
          class="inline-flex items-center gap-2 mt-6 text-[11px] tracking-[0.18em] uppercase transition-opacity hover:opacity-60"
          style="color:var(--text-2)">
          info@medlake.ch
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M1 5h8M5 1l4 4-4 4" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </a>
      </div>
    </div>
