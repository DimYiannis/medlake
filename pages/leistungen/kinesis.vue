    ...
    <template #body>
      <div class="space-y-10">
        ...
        <div class="px-7 py-6 border-l-2" style="border-color:var(--text-3);background:var(--bg-card)">
          <p class="text-[15px] tracking-[0.1em] uppercase mb-2" style="color:var(--text-3)">{{ page?.suitableLabel || '' }}</p>
          <p class="text-[17px] leading-[1.7]" style="color:var(--text-2)">{{ page?.suitableText || '' }}</p>
        </div>
      </div>
    </template>
  </ServiceLayout>
...
const { t } = useI18n()
const page = await usePageContent('leistungen/kinesis')
useHead({ title: computed(() => page.value?.title ? `${page.value.title} – Medlake` : t('pages.leistungen.kinesis.meta')) })
